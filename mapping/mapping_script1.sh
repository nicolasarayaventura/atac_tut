set -x -e  bsub -P acc_oscarlr -q premium -n 2 -W 24:00 -R "rusage[mem=8000]" -o "job1_${sample}.txt" \
...
function indexing {
	...
	bsub -P acc_oscarlr -q premium -n 2 -W 24:00 -R "rusage[mem=8000]" -o "job1_${sample}.txt" \
		"bwa mem -t 2 ${ref_gen} ${data_dir}/${sample}_tr_1P.fastq.gz \
		${data_dir}/${sample}_tr_2P.fastq.gz > ${data_dir}/indexing/${sample}.bam | #they keep in bam format? 
		samtools sort -@2 -o ${samfiles}/${sample}.bam - \ # worked good till here 
